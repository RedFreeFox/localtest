...
//			String queryString = "select p.projectusertype,p.userinfo.name from fetch Projectuser as p where p.project.projectid=? group by p.projectusertype,p.userinfo.name";
			String sql = "select p.* from userinfo u ,projectuser p where u.userid = p.projectuserid and p.projectid="+projectid+" ";
			return this.getSession().createSQLQuery(sql).addEntity(Projectuser.class).list();
		} catch (RuntimeException re) {
			log.error("find by property name failed", re);
			throw re;
		}
	}
	public List findByProjectIds(Long projectid,Long projectusertype) {
		try {
//			String queryString = "select p.projectusertype,p.userinfo.name from fetch Projectuser as p where p.project.projectid=? group by p.projectusertype,p.userinfo.name";
			String sql = "select distinct p.projectusertype,u.name from userinfo u ,projectuser p where u.userid = p.projectuserid and p.projectid="+projectid+" and p.projectusertype="+projectusertype;
	...
	public List findByUserId(Long UserId){
		...
		String sql="select projectid from ProjectUser where projectuserid="+UserId+" group by projectid";
	...
	public List findProAndType(Long ProID,Long Type) {
		log.debug("finding all Projectuser instances");
		try {
			String sql="select distinct b.* from Projectuser a,Userinfo b where a.ProjectUserID=b.userid and ProjectID="+ProID+" and ProjectUserType="+Type;
			//String queryString = "from Projectuser where ProjectID="+ProID+" and ProjectUserType="+Type;
			//return getHibernateTemplate().find(queryString);
			return this.getSession().createSQLQuery(sql).addEntity(Userinfo.class).list();
		} catch (RuntimeException re) {
			log.error("find all failed", re);
			throw re;
		}
	}
	public List findProAndTypes(Long ProID,Long Type) {
		log.debug("finding all Projectuser instances");
		try {
			String queryString = "from Projectuser where ProjectID="+ProID+" and ProjectUserType="+Type;
			return getHibernateTemplate().find(queryString);
	...
	public List findProAndUser(Long ProID,Long UserID) {
		log.debug("finding all Projectuser instances");
		try {
			String queryString = "from Projectuser where ProjectID="+ProID+" and ProjectUserID="+UserID;
			return getHibernateTemplate().find(queryString);
		} catch (RuntimeException re) {
			log.error("find all failed", re);
			throw re;
