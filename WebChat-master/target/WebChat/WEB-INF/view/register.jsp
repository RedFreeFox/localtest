...
<div class="login-form">
    ...
    <form id="login-form" action="<%=path%>/user/login" method="post">
        ...
        <div class="key" style="margin-bottom:4em">
            <input type="text" id="password" name="password" placeholder="请输入密码">
        </div>
        <div class="key" style="margin-top:-6em">
            <input type="text" id="nickname" name="nickname" placeholder="请输入姓名">
        </div>
        <div class="key" style="margin-top:-2em">
            <input type="text" id="sex" name="sexx" placeholder="请输入性别">
        </div>
        <div class="key" style="margin-top:-2em">
            <input type="text" id="age" name="age" placeholder="请输入年龄">
        </div>
        <div class="signin">
            <input type="submit" id="register" onclick="changeAction()" value="Register">
        </div>
    </form>
</div>
...
<script>
    ...
    function changeAction() {
        ...
        if (isNull(nickname)) {
            $('#submit').attr('value', '请输入名字!!!').css('background', 'red');
            return false;
        }
        if (isNull(sex)) {
            $('#submit').attr('value', '请输入性别!!!').css('background', 'red');
            return false;
        }
        if (isNull(age)) {
            $('#submit').attr('value', '请输入年龄!!!').css('background', 'red');
            return false;
        }
        $("#login-form").attr('action', '<%=path%>/user/register');
        $("#login-form").submit();
    }
</script>
</body>
